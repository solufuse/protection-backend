...
def analyze_topology(file_content: bytes, filename: str) -> dict:
    ...
    if df_ibus is not None:
        ...
        if bus_id_col:
            ...
            if bus_kv_col:
                bus_voltage_map = df_ibus.set_index(bus_id_col)[bus_kv_col].to_dict()
    ...
    # --- 6. Identify Couplings (Tie-Breakers) ---
    coupling_info = []
    if type_col and bus_voltage_map:
        for _, row in df_iconnect.iterrows():
            is_potential_coupling = False
            if type_col and pd.notna(row[type_col]) and ('CB' in row[type_col].upper() or 'TIE' in row[type_col].upper()):
                is_potential_coupling = True
            
            if is_potential_coupling:
