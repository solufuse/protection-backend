...
def calc_inrush_rms_decay(i_nom: float, ratio: float, tau_ms: float, time_s: float) -> float:
    """
    Calcule la valeur EFFICACE (RMS) du courant d'enclenchement.
    I_peak = In * Ratio
    I_rms(0) = I_peak / sqrt(2)
    I_rms(t) = I_rms(0) * exp(-t / tau)
    """
    if tau_ms <= 0: return 0.0
    tau_s = tau_ms / 1000.0
    
    # Correction : Division par sqrt(2) pour passer du Peak (Ratio) au RMS
...
def calculate(plan: ProtectionPlan, full_config: ProjectConfig, dfs_dict: dict, global_tx_map: dict) -> dict:
    ...
    if plan.type.upper() == "TRANSFORMER":
        ...
        # D. Calculs INRUSH (Decay Curve RMS corrected)
        # On utilise In_prim (Un) comme base standard
        ...
        data_settings.update({
            ...
            "inrush_50ms_Formula": f"({round(base_inrush_current,2)} * {ratio_iencl} / sqrt(2)) * exp(-0.05 / {tau_ms/1000})",
            
            "inrush_900ms": round(inrush_val_900ms, 2),
            "inrush_900ms_Formula": f"({round(base_inrush_current,2)} * {ratio_iencl} / sqrt(2)) * exp(-0.9 / {tau_ms/1000})"
        })
