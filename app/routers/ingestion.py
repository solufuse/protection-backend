...
# --- [HELPER] Resolve Path V2 ---
# [?] [THOUGHT] Standardized path resolution logic shared across V2 routers.
...
def preview_data(
    ...
    user = Depends(get_current_user), # [+] [INFO] Handles Token from Header OR Query Param
    ...
    if filename.lower().endswith('.json'):
        ...
        # [!] [CRITICAL] DB Converter extracts tables from SQLite/MDB binaries
        dfs = db_converter.extract_data_from_db(content)
        if not dfs: 
            raise HTTPException(500, "Could not extract data from DB")
        ...
        raise HTTPException(400, "Format not supported for preview")
