...
def get_config(target_id, is_project) -> ProjectConfig:
...
@router.post("/run")
async def run_ansi_51_only(include_data: bool = False, token: str = Depends(get_current_token), project_id: str = Query(None)):
    ...
    # Warning: We use 'target' as token for library compatibility, assuming lib calls session_manager.get_files(target)
    # We must ensure ansi_51.py library is updated or compatible. 
    # For now, we rely on the fact that target is passed as the key to get_files.
    full_results = ansi_51.run_batch_logic(config, target) 
    ...
    for r in full_results:
        ...
        if not include_data:
            ...
            if "common_data" in r_copy:
                ...
                ds.pop("raw_data_from", None); ds.pop("raw_data_to", None)
...
@router.get("/export")
async def export_ansi_51(format: str = "xlsx", token: str = Depends(get_current_token), project_id: str = Query(None)):
    target, is_proj = (project_id, True) if project_id else (token, False)
    if is_proj and not session_manager.can_access_project(token, project_id): raise HTTPException(403, "Access Denied")

    config = get_config(target, is_proj)
    results = ansi_51.run_batch_logic(config, target)
